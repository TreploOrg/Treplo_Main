...
public class SearchServiceClient : ISearchServiceClient
{
    ...
    public async IAsyncEnumerable<TrackSearchResult> SearchAsync(string query, uint? limit = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        var request = $"{requestUri}?query={query}";
        if (limit is { } l)
            request += $"&limit={l}";
        await using var resultStream = await httpClient.GetStreamAsync(request, cancellationToken);
        await foreach (var track in JsonSerializer.DeserializeAsyncEnumerable<TrackSearchResult>(resultStream, jsonSerializerOptions, cancellationToken: cancellationToken))
        {
            yield return track;
        }
    }
...
{
    IAsyncEnumerable<TrackSearchResult> SearchAsync(string query, uint? limit = null, CancellationToken cancellationToken = default);
}